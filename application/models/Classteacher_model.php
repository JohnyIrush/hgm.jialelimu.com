...
class Classteacher_model extends MY_Model {
    ...
    function teacherByClassSection($class_id, $section_id) {
        ...
        $query = $this->db->select('staff.*,class_teacher.id as ctid,class_teacher.class_id,class_teacher.section_id,class.classes,streams.stream')->join("staff", "class_teacher.staff_id = staff.id")->join("class", "class_teacher.class_id = class.classesID")->join("streams", "class_teacher.section_id = streams.Streamid")->where("class_teacher.class_id", $class_id)->where("class_teacher.section_id", $section_id)->where("staff.is_active", 1)->where("class_teacher.session_id", $this->current_session)->get("class_teacher");
